(function (window){
  ...
  var Saya = window.Saya || {};
  window.Saya = window.Saya || Saya;
    
  Saya.MathUtil = {};
  
  //used for radiansToDegrees and degreesToRadians
  Saya.MathUtil.PI_180 = Math.PI/180;
  Saya.MathUtil.ONE80_PI = 180/Math.PI;
  
  //precalculations for values of 90, 270 and 360 in radians
  Saya.MathUtil.PI2 = Math.PI*2;
  Saya.MathUtil.HALF_PI = Math.PI/2;


  //return number between 1 and 0
  Saya.MathUtil.normalize = function(value, minimum, maximum){
    return (value - minimum) / (maximum - minimum);
  };

  //map normalized number to values
  Saya.MathUtil.interpolate = function(normValue, minimum, maximum){
    return minimum + (maximum - minimum) * normValue;
  };

  //map a value from one set to another
  Saya.MathUtil.map = function(value, min1, max1, min2, max2){
    return Saya.MathUtil.interpolate( Saya.MathUtil.normalize(value, min1, max1), min2, max2);
  };

  Saya.MathUtil.getRandomNumberInRange = function(min, max){
    return min + Math.random() * (max - min);
  };
  
  Saya.MathUtil.getRandomIntegerInRange = function(min, max){
    return Math.round(Saya.MathUtil.getRandomNumberInRange(min, max));
  };
...
(function (window){
    ...
    Saya.Geom.Point = function (x,y){
        this.x = isNaN(x) ? 0 : x;
        this.y = isNaN(y) ? 0 : y;
    };

    Saya.Geom.Point.prototype.clone = function(){
        return new Saya.Geom.Point(this.x,this.y);
    };

    Saya.Geom.Point.prototype.update = function(x, y){
        this.x = isNaN(x) ? this.x : x;
        this.y = isNaN(y) ? this.y : y;
    };

    Saya.Geom.Point.prototype.equals = function(point){
        return this.x==point.x && this.y==point.y;
    };

    Saya.Geom.Point.prototype.toString = function(){
  ...
  Saya.Geom.Rectangle = function (x, y, width, height){
    this.update(x, y, width, height);
  };
  
  Saya.Geom.Rectangle.prototype.update = function(x, y, width, height){
    this.x = isNaN(x) ? 0 : x;
    this.y = isNaN(y) ? 0 : y;
  ...
  Saya.Geom.Rectangle.prototype.getRight = function(){
    return this.x + this.width;
  };
  
  Saya.Geom.Rectangle.prototype.getBottom = function(){
    return this.y + this.height;
  };

    Saya.Geom.Rectangle.prototype.getCenterX = function(){
        return this.x + this.width/2;
    };

    Saya.Geom.Rectangle.prototype.getCenterY = function(){
        return this.y + this.height/2;
    };

    Saya.Geom.Rectangle.prototype.containsPoint = function(x, y){
        return x >= this.x && y >= this.y && x <= this.getRight() && y <= this.getBottom();
    };

  
  Saya.Geom.Rectangle.prototype.clone = function(){
    return new Sakri.Geom.Rectangle(this.x, this.y, this.width, this.height);
  };
  
  Saya.Geom.Rectangle.prototype.toString = function(){
...
(function (window){
    ...
    Saya.CanvasTextUtil.getFontSizeForRect = function(string, fontProps, rect, canvas, fillStyle){
        if(!canvas){
    ...
    Saya.CanvasTextProperties = function(fontWeight, fontStyle, fontSize, fontFace){
    ...
    Saya.CanvasTextProperties.NORMAL = "normal";
    Saya.CanvasTextProperties.BOLD = "bold";
    Saya.CanvasTextProperties.BOLDER = "bolder";
    Saya.CanvasTextProperties.LIGHTER = "lighter";

    Saya.CanvasTextProperties.ITALIC = "italic";
    Saya.CanvasTextProperties.OBLIQUE = "oblique";


    Saya.CanvasTextProperties.prototype.setFontWeight = function(fontWeight){
        switch (fontWeight){
            case Saya.CanvasTextProperties.NORMAL:
            case Saya.CanvasTextProperties.BOLD:
            case Saya.CanvasTextProperties.BOLDER:
            case Saya.CanvasTextProperties.LIGHTER:
                this.fontWeight = fontWeight;
                break;
            default:
                this.fontWeight = Saya.CanvasTextProperties.NORMAL;
        }
    };

    Saya.CanvasTextProperties.prototype.setFontStyle = function(fontStyle){
        switch (fontStyle){
            case Saya.CanvasTextProperties.NORMAL:
            case Saya.CanvasTextProperties.ITALIC:
            case Saya.CanvasTextProperties.OBLIQUE:
                this.fontStyle = fontStyle;
                break;
            default:
                this.fontStyle = Saya.CanvasTextProperties.NORMAL;
        }
    };

    Saya.CanvasTextProperties.prototype.setFontSize = function(fontSize){
    ...
    Saya.CanvasTextProperties.prototype.clone = function(){
        return new Saya.CanvasTextProperties(this.fontWeight, this.fontStyle, this.fontSize, this.fontFace);
    };

    Saya.CanvasTextProperties.prototype.getFontString = function(){
...
function commitResize(){
    ...
    htmlBounds = new Saya.Geom.Rectangle(0,0, getWidth(canvasContainer) , getHeight(canvasContainer));
    ...
    bounds = new Saya.Geom.Rectangle(0,0, canvas.width, canvas.height);
    context.clearRect(0,0,canvas.width, canvas.height);
...
//========================
    ...
    var scrollSpeed = 3;
    var score;
    var fontProperties = new Saya.CanvasTextProperties(Saya.CanvasTextProperties.BOLD, null, 100);
    ...
    function createLogo(logoText, logoCanvas, logoCanvassBG){
        ...
        var textRect = new Saya.Geom.Rectangle(0, 0, logoCanvas.width * .8, logoCanvas.height);
        var logoFontProps = fontProperties.clone();
        logoFontProps.fontSize = Saya.CanvasTextUtil.getFontSizeForRect(logoText, fontProperties, textRect);
    ...
    var collisionPoint = new Saya.Geom.Point();
    var birdPoints = [];
    ...
    var characters;
    var birdFontProperties = new Saya.CanvasTextProperties(Saya.CanvasTextProperties.BOLD, null, 50);
    ...
    function createTubes(){
        ...
        for(var i = 0; i < totalTubes; i++){
            ...
            tubes[i].topRect = new Saya.Geom.Rectangle(canvas.width+(i * tubesGapWidth));
            tubes[i].bottomRect = new Saya.Geom.Rectangle(canvas.width+(i * tubesGapWidth));
            renderTube(tubes[i]);
        }
        currentTube = tubes[0];
    }
...
function createCityGraphic(){
    ...
    for(iterator=0; iterator<canvas.width; iterator+=minCloudRadius){
        cgContext.beginPath();
        cgContext.arc( iterator , maxCloudRadius, Saya.MathUtil.getRandomNumberInRange(minCloudRadius, maxCloudRadius), 0, Saya.MathUtil.PI2);
        cgContext.closePath();
        cgContext.fill();
    }
    ...
    for(iterator=0; iterator<canvas.width; iterator+=(houseWidth+8)){
        ...
        houseHeight = Saya.MathUtil.getRandomNumberInRange(cityGraphicHeight *.5 , cityGraphicHeight - maxCloudRadius *.8);
    ...
    for(iterator=0; iterator<canvas.width; iterator+=minTreeRadius){
        cgContext.beginPath();
        radius = Saya.MathUtil.getRandomNumberInRange(minCloudRadius, maxCloudRadius)
        cgContext.arc( iterator , cityGraphicHeight, radius, 0, Saya.MathUtil.PI2);
        cgContext.closePath();
        cgContext.fill();
    ...
    var sand = "#dcd795";
    var groundGraphicRect = new Saya.Geom.Rectangle();
